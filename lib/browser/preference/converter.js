...
exports.isDiffEditorOption = exports.isEditorOption = exports.diffEditorOptionsConverters = exports.textModelUpdateOptionsConverters = exports.editorOptionsConverters = exports.getConvertedMonacoOptions = void 0;
const ide_core_browser_1 = require("@opensumi/ide-core-browser");
const { removeUndefined } = ide_core_browser_1.objects;
/**
 ...
 */
function getConvertedMonacoOptions(configurationService, resourceUri, language, updatingKey) {
    const editorOptions = {};
    const diffOptions = {};
    const modelOptions = {};
    const editorOptionsKeys = updatingKey
        ? updatingKey.filter((key) => exports.editorOptionsConverters.has(key))
        : Array.from(exports.editorOptionsConverters.keys());
    const textModelUpdateOptionsKeys = updatingKey
        ? updatingKey.filter((key) => exports.textModelUpdateOptionsConverters.has(key))
        : Array.from(exports.textModelUpdateOptionsConverters.keys());
    const diffEditorOptionsKeys = updatingKey
        ? updatingKey.filter((key) => exports.diffEditorOptionsConverters.has(key))
        : Array.from(exports.diffEditorOptionsConverters.keys());
    editorOptionsKeys.forEach((key) => {
        const value = configurationService.getValue(key, {
            resource: resourceUri ? ide_core_browser_1.Uri.parse(resourceUri) : undefined,
            overrideIdentifier: language,
        });
        if (value === undefined) {
            return;
        }
        if (!exports.editorOptionsConverters.get(key)) {
            editorOptions[key] = value;
        }
        else {
            const converter = exports.editorOptionsConverters.get(key);
    ...
    textModelUpdateOptionsKeys.forEach((key) => {
        ...
        if (!exports.textModelUpdateOptionsConverters.get(key)) {
            modelOptions[key] = value;
        }
        else {
            const converter = exports.textModelUpdateOptionsConverters.get(key);
    ...
    diffEditorOptionsKeys.forEach((key) => {
        ...
        if (!exports.diffEditorOptionsConverters.get(key)) {
            editorOptions[key] = value;
        }
        else {
            const converter = exports.diffEditorOptionsConverters.get(key);
...
exports.getConvertedMonacoOptions = getConvertedMonacoOptions;
/**
 * Configuration options for the editor.
 */
exports.editorOptionsConverters = new Map([
    /**
...
exports.textModelUpdateOptionsConverters = new Map([
...
exports.diffEditorOptionsConverters = new Map([
    /**
...
function isContainOptionKey(key, optionMap) {
...
function isEditorOption(key) {
    return (isContainOptionKey(key, exports.editorOptionsConverters) ||
        isContainOptionKey(key, exports.textModelUpdateOptionsConverters) ||
        isContainOptionKey(key, exports.diffEditorOptionsConverters));
}
exports.isEditorOption = isEditorOption;
function isDiffEditorOption(key) {
    return isContainOptionKey(key, exports.diffEditorOptionsConverters);
}
exports.isDiffEditorOption = isDiffEditorOption;
