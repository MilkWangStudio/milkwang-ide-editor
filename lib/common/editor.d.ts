...
import { URI, Event, BasicEvent, IDisposable, MaybeNull, IRange, ISelection, ILineChange, IPosition, IThemeColor, IMarkdownString } from '@opensumi/ide-core-common';
...
export interface IEditor {
     ...
     * @param opts
     */
    insertSnippet(template: string, ranges: readonly IRange[], opts: IUndoStopOptions): any;
    /**
     * 应用装饰器
     * @param key
     * @param options
     */
    applyDecoration(key: string, options: IDecorationApplyOptions[]): any;
    ...
    setSelections(selection: IRange[] | ISelection[]): any;
    setSelection(selection: IRange | ISelection): any;
    updateOptions(editorOptions?: IEditorOptions, modelOptions?: ITextModelUpdateOptions): any;
    save(): Promise<void>;
    /**
     * 获得包裹的 monaco 编辑器
     */
...
export interface IDiffEditor extends IDisposable {
    compare(originalDocModelRef: IEditorDocumentModelRef, modifiedDocModelRef: IEditorDocumentModelRef, options?: IResourceOpenOptions, rawUri?: URI): any;
...
export declare abstract class EditorCollectionService {
    /**
     * 当前的编辑器
     */
    readonly currentEditor: IEditor | undefined;
    /**
     * 创建一个 monaco 编辑器实例
     * @param dom
     * @param options
     * @param overrides
     */
    abstract createCodeEditor(dom: HTMLElement, options?: any, overrides?: {
        [key: string]: any;
    }): ICodeEditor;
    /**
     ...
     * @param overrides
     */
    abstract createDiffEditor(dom: HTMLElement, options?: any, overrides?: {
        [key: string]: any;
    }): IDiffEditor;
    abstract createMergeEditor(dom: HTMLElement, options?: any, overrides?: {
        [key: string]: any;
    }): IMergeEditorEditor;
    abstract listEditors(): IEditor[];
    abstract listDiffEditors(): IDiffEditor[];
    abstract onCodeEditorCreate: Event<ICodeEditor>;
    abstract onDiffEditorCreate: Event<IDiffEditor>;
}
export type IOpenResourceResult = {
    group: IEditorGroup;
    resource: IResource;
} | false;
/**
 * 当前显示的Editor列表发生变化时
 */
export declare class CollectionEditorsUpdateEvent extends BasicEvent<IEditor[]> {
}
/**
 * 当EditorGroup中打开的uri发生改变时
 */
export declare class DidChangeEditorGroupUriEvent extends BasicEvent<URI[][]> {
}
/**
 * 当 Decoration Provider 收集完 monaco decoration option 并设置后
 */
export declare class DidApplyEditorDecorationFromProvider extends BasicEvent<{
    key?: string;
    uri: URI;
}> {
}
...
export interface IEditorGroup {
    ...
    restoreState(IEditorGroupState: any): Promise<void>;
...
export declare abstract class WorkbenchEditorService {
    /**
     * 当前 resource 发生变更
     */
...
export declare enum IEditorPriority {
    builtin = "builtin",
    option = "option",
    exclusive = "exclusive",
    default = "default"
}
export declare enum EditorOpenType {
    code = "code",
    diff = "diff",
...
export interface IEditorOpenType {
     ...
     * @param resource
     */
    saveResource?(resource: IResource, reason: SaveReason): any;
    /**
     * 用户调用回滚文档时，如果是这个打开方式，则调用相关指令
     * @param resource
     */
    revertResource?(resource: IResource): any;
    undo?(resource: IResource): any;
    redo?(resource: IResource): any;
}
export declare enum DragOverPosition {
    LEFT = "left",
    RIGHT = "right",
    TOP = "top",
    BOTTOM = "bottom",
...
export declare enum EditorGroupSplitAction {
    Top = 1,
    Bottom = 2,
    Left = 3,
    Right = 4
...
export declare namespace AUTO_SAVE_MODE {
    const OFF = "off";
    const AFTER_DELAY = "afterDelay";
    const EDITOR_FOCUS_CHANGE = "editorFocusChange";
    const WINDOWS_LOST_FOCUS = "windowLostFocus";
}
...
export declare function getSimpleEditorOptions(): IEditorOptions;
/**
